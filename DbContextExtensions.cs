...
using Damasio34.GraphDiff.Extensions;
...
namespace Damasio34.GraphDiff
{
    public static class DbContextExtensions
    {
        ...
        /// <typeparam name="TProp">The type of the entity member configured.</typeparam>
        /// <param name="entity">The entity.</param>
        /// <param name="mapping">The relations specified by an IUpdateConfiguration mapping</param>
        public static void UpdateGraph<T>(
            this DbContext context, T entity,
            Expression<Func<IUpdateConfiguration<T>, object>> mapping)
            where T : class
        {
            context.UpdateGraph<T, object>(entity, mapping);
        }
        ...
        /// <param name="entity">The entity.</param>
        /// <param name="mapping">The relations specified by an IUpdateConfiguration mapping</param>
        public static void UpdateGraph<T, TProp>(this DbContext context, T entity, Expression<Func<IUpdateConfiguration<T>, TProp>> mapping) 
            where T : class
            where TProp : class
        {
            // Guard null mapping
        ...
        private static void RecursiveGraphUpdate(DbContext context, object dataStoreEntity, object updatingEntity, UpdateMember member)
        {
            ...
            if (member.IsCollection)
            {
                ...
                if (actualEntityIsOnTheContext && context.Entry(updatingEntity).State != EntityState.Added)
                {
                    ...
                    member.SetValue(updatingEntity, updateValues);
                }
                else
                {
            ...
            else // not collection
            {
                ...
                if (!member.IsOwned)
                {
                    ...
                    if (newvalue == null)
                    {
                        member.SetValue(dataStoreEntity, null);
                        return;
                    }
                    ...
                    member.SetValue(dataStoreEntity, newvalue);
